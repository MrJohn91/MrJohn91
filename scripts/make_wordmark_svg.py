#!/usr/bin/env python3
"""Hand-authored neofetch-style info card SVG. Logo lines fade in top to
bottom, then the whole card gently rocks side to side."""
import sys
...
def build_svg() -> str:
    ...
    # left: ASCII logo, fully visible by default. The gentle rock is a CSS
    # animation layered on top, safe to lose since the content never
    # depends on the animation to become visible (GitHub renders these via
    # <img>, which most browsers, Chrome included, do not animate).
    parts.append(f'<g class="rock" style="transform-origin:{cx:.0f}px {cy:.0f}px" transform="translate({PAD},{PAD})">')
    for i, line in enumerate(LOGO):
        y = (i + 1) * LOGO_LH - 3
        parts.append(f'<text class="logo" x="0" y="{y:.1f}">{esc(line)}</text>')
    parts.append('</g>')

    # right: neofetch-style field list
    fx = logo_w + PAD * 2
    parts.append(f'<g transform="translate({fx:.0f},{PAD})">')
    for i, (k, v) in enumerate(FIELDS):
        y = (i + 1) * FIELD_LH - 6
        parts.append(f'<text class="k" x="0" y="{y:.1f}">{esc(k)}</text>')
        parts.append(f'<text class="v" x="150" y="{y:.1f}">{esc(v)}</text>')
    parts.append('</g>')
